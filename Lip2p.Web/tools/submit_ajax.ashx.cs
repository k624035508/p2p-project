    ...
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        private void user_register(HttpContext context)
        {
            ...
            else
            {
                ...
                //防止Session提前过期
                Utils.WriteCookie(DTKeys.COOKIE_USER_NAME_REMEMBER, "Lip2p", model.user_name, true);
                Utils.WriteCookie(DTKeys.COOKIE_USER_PWD_REMEMBER, "Lip2p", model.password, true);
        ...
        private void order_save(HttpContext context)
        {
            ...
            //检查购物车商品
            IList<Model.cart_items> iList = Lip2p.Web.UI.ShopCart.GetList(user_group_id);
            if (iList == null)
            {
            ...
            //统计购物车
            Model.cart_total cartModel = Lip2p.Web.UI.ShopCart.GetTotal(user_group_id);
            //保存订单=======================================================================
            ...
            if (!string.IsNullOrEmpty(user.pay_password))
            {
                try
                {
                    ...
                    if (Utils.MD5(pw).Equals(user.pay_password))
                    {
                        ...
                        if (DateTime.Now.Date <= new DateTime(2015, 7, 12) && proj.tag != (int)Lip2pEnums.ProjectTagEnum.Trial && proj.tag != (int)Lip2pEnums.ProjectTagEnum.DailyProject)
                        {
        ...
        private void add_bank_card(HttpContext context)
        {
            ...
            var linq_context = new Lip2pDataContext();
            try
            {
        ...
        private void search_idcard(HttpContext context)
        {
            try
            {
                ...
                var licontext = new Lip2pDataContext();
                // 判断身份证是否重复
