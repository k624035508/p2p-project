...
namespace Lip2p.Web.tools
{
    ...
    public class admin_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (action)
            {
                ...
                case "navigation_validate": //验证导航菜单ID是否重复
                    ...
                    break;
                case "department_validate": //验证部门名称是否重复
                    department_validate(context);
                    break;
        ...
        #region 验证部门名称是否重复==========================
        private void department_validate(HttpContext context)
        {
            string navname = DTRequest.GetString("param");
            string old_name = DTRequest.GetString("old_name");
            if (string.IsNullOrEmpty(navname))
            {
                context.Response.Write("{ \"info\":\"该部门名称不可为空！\", \"status\":\"n\" }");
                return;
            }
            if (navname.ToLower() == old_name.ToLower())
            {
                context.Response.Write("{ \"info\":\"该部门名称可使用\", \"status\":\"y\" }");
                return;
            }
            //检查保留的名称开头
            if (navname.ToLower().StartsWith("channel_"))
            {
                context.Response.Write("{ \"info\":\"该部门名称系统保留，请更换！\", \"status\":\"n\" }");
                return;
            }
            BLL.department bll = new BLL.department();
            if (bll.Exists(navname))
            {
                context.Response.Write("{ \"info\":\"该部门名称已被占用，请更换！\", \"status\":\"n\" }");
                return;
            }
            context.Response.Write("{ \"info\":\"该部门名称可使用\", \"status\":\"y\" }");
            return;
        ...
        {
            Model.manager adminModel = ManagePage.GetAdminInfo(); //获得当前登录管理员信息
            if (adminModel == null)
            {
                return;
            }
        ...
        {
            //取得管理员登录信息
            Model.manager adminInfo = ManagePage.GetAdminInfo();
            if (adminInfo == null)
            {
                    ...
                    if (orderConfig.confirmmsg > 0)
                    {
                        switch (orderConfig.confirmmsg)
                        {
                            case 1: //短信通知
                                ...
                                bool sendStatus = SMSHelper.Send(model.mobile, msgContent, 2, out tipMsg);
                                if (!sendStatus)
                                {
        ...
        private void sms_message_post(HttpContext context)
        {
            ...
            bool result = SMSHelper.Send(mobiles, content, 2, out msg);
            if (result)
            {
