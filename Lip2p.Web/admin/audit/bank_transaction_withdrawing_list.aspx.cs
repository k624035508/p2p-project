...
namespace Lip2p.Web.admin.audit
{
       ...
       private IQueryable<li_bank_transactions> QueryWithdraws()
        {
            ...
            var query = context.li_bank_transactions.Where(b => b.type == (int) (Lip2pEnums.BankTransactionTypeEnum.Withdraw));
            ...
            //{
            //    if (Convert.ToInt32(rblBankTransactionStatus.SelectedValue) == (int)Lip2pEnums.BankTransactionStatusEnum.Acting)
            //    {
            //        query = query.Where(q => (q.create_time >= DateTime.Parse("2015-7-6")));
            //        query = query.Where(b => b.status == (int)Lip2pEnums.BankTransactionStatusEnum.Acting);
            //    }
            //    else if (Convert.ToInt32(rblBankTransactionStatus.SelectedValue) == (int)Lip2pEnums.BankTransactionStatusEnum.Confirm)
            //    {
            //        query = query.Where(q => (q.create_time < DateTime.Parse("2015-7-6")));
            //        query = query.Where(b => b.status != (int)Lip2pEnums.BankTransactionStatusEnum.Cancel);
            //    }
            //    else
            //        query = query.Where(b => b.status == (int)Lip2pEnums.BankTransactionStatusEnum.Cancel);
            //}
        ...
        {
            return tr.handling_fee_type == (int)Lip2pEnums.BankTransactionHandlingFeeTypeEnum.NoHandlingFee
                ? 0
        ...
        protected void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            var lsData = withdraws.OrderBy(q => q.status).ThenByDescending(q => q.transact_time)
                ...
                .Select(bt => new
                {
                    ...
                    status = Utils.GetLip2pEnumDes((Lip2pEnums.BankTransactionStatusEnum)bt.status),
