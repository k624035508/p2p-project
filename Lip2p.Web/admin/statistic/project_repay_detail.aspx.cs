        ...
        private List<RepaymentDetail> QueryRepayDetails()
        {
            ...
            if (rblRepaymentTaskStatus.SelectedValue != "0")
            {
                if (rblRepaymentTaskStatus.SelectedValue == "2") // 已还款
                    query = query.Where(r => (int) Lip2pEnums.RepaymentStatusEnum.ManualPaid <= r.status);
                else
            ...
            var appendedSums =
                query.OrderBy(r => r.should_repay_time)
                    ...
                    .SelectMany(ri =>
                    {
                        ...
                        if (ri.repayment.status != (int) Lip2pEnums.RepaymentStatusEnum.Unpaid)
                        {
                            // 查询所有收益记录
                            profiting = pro.li_project_transactions.Where(
                                t =>
                                    t.create_time == r.repay_at && t.type != (int)Lip2pEnums.ProjectTransactionTypeEnum.Invest &&
                                    t.status == (int)Lip2pEnums.ProjectTransactionStatusEnum.Success).ToList();
                        }
                        else
                        {
                        ...
                        repaymentDetails.First().RepaymentTask = new RepaymentTaskDetail
                        {
                            ...
                            CreditorName = pro.li_risks.li_creditors.real_name,
                            FinancingAmount = pro.financing_amount,
                            ProfitRateYear = pro.profit_rate_year.ToString(),
                            Term =
                                pro.repayment_term_span_count + " " +
                                Utils.GetLip2pEnumDes((Lip2pEnums.ProjectRepaymentTermSpanEnum) pro.repayment_term_span),
