<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="project_investment_list.aspx.cs" Inherits="Lip2p.Web.admin.transact.project_investment_list" %>
...
<ItemTemplate>
  <tr>
    ...
    <td><%# Utils.GetLip2pEnumDes((Lip2pEnums.ProjectTransactionTypeEnum) Convert.ToByte(Eval("type"))) %></td>
    <td><%# Utils.GetLip2pEnumDes((Lip2pEnums.ProjectTransactionStatusEnum) Convert.ToByte(Eval("status"))) %></td>
    ...
    <td><asp:Button ID="btnRefund" runat="server" Text="退款" UseSubmitBehavior="False" CommandArgument='<%# Eval("id") %>' OnClientClick="return ExeNoCheckPostBack(this.name, '确认退款？');"
            OnClick="btnRefund_OnClick" Visible='<%# Convert.ToByte(Eval("status")) == (int) Lip2pEnums.ProjectTransactionStatusEnum.Success
                      && Convert.ToByte(Eval("li_projects.status")) == (byte) Lip2pEnums.ProjectStatusEnum.FaBiao %>' /></td>
  </tr>
</ItemTemplate>
<FooterTemplate>
