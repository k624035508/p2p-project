<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="project_repayment_list.aspx.cs" Inherits="Lip2p.Web.admin.transact.project_repayment_list" %>
...
<ItemTemplate>
  <tr>
    ...
    <td><%# Utils.GetLip2pEnumDes((Lip2pEnums.RepaymentStatusEnum) Convert.ToByte(Eval("status")))%></td>
    ...
    <td><asp:Button ID="btnRepayNow" runat="server" Text="马上放款" UseSubmitBehavior="False" CommandArgument='<%# Eval("id") %>' OnClientClick="return ExeNoCheckPostBack(this.name, '确认进行放款？');"
        OnClick="btnRepayNow_OnClick" Visible='<%# Convert.ToByte(Eval("status")) == (byte) Lip2pEnums.RepaymentStatusEnum.Unpaid && (Utils.IsDebugging() || Convert.ToDateTime(Eval("should_repay_time")).Date <= DateTime.Today)%>'/></td>
  </tr>
</ItemTemplate>
<FooterTemplate>
