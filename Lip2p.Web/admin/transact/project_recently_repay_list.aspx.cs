...
namespace Lip2p.Web.admin.transact
{
    public partial class project_recently_repay_list : UI.ManagePage
    {
            ...
            if (rblProjectShowType.SelectedValue == "1")
            {
                query = query.Where(p =>
                            p.li_repayment_tasks.Any(r =>
                                    r.status == (int)Lip2pEnums.RepaymentStatusEnum.Unpaid &&
        ...
        protected void btnRepayNow_OnClick(object sender, EventArgs e)
        {
            ...
            try
            {
                var repaymentTaskId = context.li_projects.Single(p => p.id == projectId)
                    .li_repayment_tasks.First(t => t.status == (int)Lip2pEnums.RepaymentStatusEnum.Unpaid && t.should_repay_time.Date <= DateTime.Today).id;
        ...
        {
            return pro.li_repayment_tasks.Any(r => r.status == (int) Lip2pEnums.RepaymentStatusEnum.Unpaid && r.should_repay_time.Date <= DateTime.Today);
        }
    }
}