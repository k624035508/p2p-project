    ...
    public class RegHongBao
    {
        ...
        private static void HandleUserLoginMsg(int userId)
        {
            ...
            var hongbaos = user.li_activity_transactions.Where(
                t =>
                    t.activity_type == (int)Agp2pEnums.ActivityTransactionActivityTypeEnum.HongBaoActivation &&
                    t.status == (int) Agp2pEnums.ActivityTransactionStatusEnum.Acting).AsEnumerable().Select(a => new RedPackage(a)).ToList();
            hongbaos.ForEach(h =>
            {
        ...
        private static void HandleUserInvestedMsg(int projectTransactionId, DateTime investTime)
        {
            ...
            var unactived = context.li_activity_transactions.Where(
                a =>
                    ...
                    a.activity_type == (int) Agp2pEnums.ActivityTransactionActivityTypeEnum.HongBaoActivation).ToList();
