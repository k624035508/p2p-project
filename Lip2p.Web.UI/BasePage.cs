...
namespace Lip2p.Web.UI
{
    public partial class BasePage : System.Web.UI.Page
    {
        ...
        public string linkurl(string _key, params object[] _params)
        {
            ...
            string currRequestPath = HttpContext.Current.Request.RawUrl.ToLower(); //当前的URL地址
        ...
        public static bool IsUserLogin()
        {
            ...
            {
                //检查Cookies
                string username = Utils.GetCookie(DTKeys.COOKIE_USER_NAME_REMEMBER, "Lip2p");
                string password = Utils.GetCookie(DTKeys.COOKIE_USER_PWD_REMEMBER, "Lip2p");
        ...
        /// 取得用户信息
        /// </summary>
        public static Model.users GetUserInfo()
        {
            if (IsUserLogin())
            {
        ...
        public static dt_users GetUserInfoByLinq()
        {
            if (IsUserLogin())
            {
                ...
                {
                    //为了能查询到最新的用户信息，必须查询最新的用户资料
                    return new Lip2pDataContext().dt_users.FirstOrDefault(u => u.id == model.id);
                }
            }
            return null;
        }
