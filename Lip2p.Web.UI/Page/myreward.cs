...
namespace Lip2p.Web.UI.Page
{
    public class myreward : usercenter
    {
        ...
        protected override void InitPage()
        {
            ...
            // 查询自己的奖励
            myReward = context.li_activity_transactions.Where(tr => tr.activity_type == (int)Lip2pEnums.ActivityTransactionActivityTypeEnum.RefereeFirstTimeProfitBonus)
        ...
        protected string getMyReward(li_invitations inv)
        {
            if (myReward.ContainsKey(inv.user_id))
            {
                ...
                return atr.value.ToString("c") + (atr.status == (int)Lip2pEnums.ActivityTransactionStatusEnum.Confirm ? "（已发放）" : "（待发放）");
            }
            return "";
        }
