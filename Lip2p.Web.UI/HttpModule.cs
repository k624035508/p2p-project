...
namespace Lip2p.Web.UI
{
    ...
    public class HttpModule : System.Web.IHttpModule
    {
                    ...
                    if (model.page == requestPath.Substring(requestPath.LastIndexOf("/") + 1))
                    {
                        ...
                        if (model.type == DTKeys.DIRECTORY_REWRITE_PLUGIN)
                        {
                            context.RewritePath(string.Format("{0}{1}/{2}{3}",
                                siteConfig.webpath, DTKeys.DIRECTORY_REWRITE_ASPX, DTKeys.DIRECTORY_REWRITE_PLUGIN, CutStringPath(siteConfig.webpath, "", requestPath)));
                            return;
                        }
                        else
                        {
                            ...
                            context.RewritePath(string.Format("{0}{1}/{2}{3}",
                                siteConfig.webpath, DTKeys.DIRECTORY_REWRITE_ASPX, defaultPath, CutStringPath(siteConfig.webpath, defaultPath, requestPath)));
                            return;
                        }
                    }
                }
                    ...
                    foreach (Model.url_rewrite_item item in model.url_rewrite_items)
                    {
                            ...
                            || (model.page == "index.aspx" && Regex.IsMatch(requestPath, string.Format("^{0}{1}$", patternPath, item.pattern), RegexOptions.None | RegexOptions.IgnoreCase)))
                        {
                                ...
                                (model.channel.Length > 0 || model.page.ToLower() == "index.aspx")) //频道页
                            {
                                if (domainPath == string.Empty && categoryPath != string.Empty)
                                {
                                    context.RewritePath(siteConfig.webpath + DTKeys.DIRECTORY_REWRITE_HTML +
                                        Utils.GetUrlExtension(CutStringPath(siteConfig.webpath, "", requestPath), siteConfig.staticextension, true));
                                }
                                else
                                {
                                    context.RewritePath(siteConfig.webpath + DTKeys.DIRECTORY_REWRITE_HTML + "/" + defaultPath +
                                        Utils.GetUrlExtension(CutStringPath(siteConfig.webpath, defaultPath, requestPath), siteConfig.staticextension, true));
                                }
                                return;
                            }
        ...
        /// <param name="dirPath">子站点目录</param>
        /// <param name="requestPath">当前页面路径</param>
        /// <returns>String</returns>
        private string CutStringPath(string webPath, string dirPath, string requestPath)
        {
            if (requestPath.StartsWith(webPath))
            {
            ...
            dirPath += "/";
            if (requestPath.StartsWith(dirPath))
            {
                requestPath = requestPath.Substring(dirPath.Length);
            }
            return "/" + requestPath;
        }
