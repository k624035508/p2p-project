...
namespace Lip2p.Core
{
    ...
    public static class TransactionFacade
    {
        ...
        /// <returns></returns>
        public static li_bank_transactions Charge(this Lip2pDataContext context, int userId, decimal money, Lip2pEnums.PayApiTypeEnum payApi, string remark = null)
        {
            // 创建交易记录（充值进行中）
            var tr = new li_bank_transactions
            {
                charger = userId,
                transact_time = null,
                type = (int)Lip2pEnums.BankTransactionTypeEnum.Charge,
                status = (int)Lip2pEnums.BankTransactionStatusEnum.Acting,
                value = money,
                handling_fee = 0,
                handling_fee_type = (byte) Lip2pEnums.BankTransactionHandlingFeeTypeEnum.NoHandlingFee,
            ...
            // 创建交易历史
            var his = CloneFromWallet(wallet, Lip2pEnums.WalletHistoryTypeEnum.Charging);
        ...
        /// <param name="remark"></param>
        /// <returns></returns>
        public static li_bank_transactions Withdraw(this Lip2pDataContext context, int bankAccountId, decimal withdrawMoney, string remark = null)
        {
            // 提现 100 起步，5w 封顶
            ...
            var tr = new li_bank_transactions
            {
                ...
                transact_time = null,
                type = (int) Lip2pEnums.BankTransactionTypeEnum.Withdraw,
                status = (int) Lip2pEnums.BankTransactionStatusEnum.Acting,
                value = withdrawMoney,
                ...
                handling_fee_type =
                    (byte)
                        (unusedMoney == 0
                            ? Lip2pEnums.BankTransactionHandlingFeeTypeEnum.WithdrawHandlingFee
                            : Lip2pEnums.BankTransactionHandlingFeeTypeEnum.WithdrawUnusedMoneyHandlingFee),
                no_order = Utils.GetOrderNumberLonger(),
                remarks = remark,
            ...
            // 创建交易历史
            var his = CloneFromWallet(wallet, Lip2pEnums.WalletHistoryTypeEnum.Withdrawing);
            his.li_bank_transactions = tr;
            context.li_wallet_histories.InsertOnSubmit(his);
        ...
        /// <param name="withdrawMoney"></param>
        /// <returns></returns>
        public static decimal CalcStandGuardFee(this Lip2pDataContext context, int userId, decimal withdrawMoney)
        {
        ...
        /// <returns></returns>
        public static li_bank_transactions ConfirmBankTransaction(this Lip2pDataContext context, int bankTransactionId, int? approver, bool saveChange = true)
        {
            // 更新原事务（完成事务）
            var tr = context.li_bank_transactions.Single(t => t.id == bankTransactionId);
            if (tr.status != (int) Lip2pEnums.BankTransactionStatusEnum.Acting)
                throw new InvalidOperationException("该银行卡" + Utils.GetLip2pEnumDes((Lip2pEnums.BankTransactionTypeEnum) tr.type) + "事务已经被确认或取消了");
            tr.status = (byte) Lip2pEnums.BankTransactionStatusEnum.Confirm;
            tr.transact_time = DateTime.Now;
            tr.approver = approver;

            if (tr.type == (int) Lip2pEnums.BankTransactionTypeEnum.Charge) // 充值确认
            {
                var wallet = tr.dt_users.li_wallets;
                // 修改钱包金额
                //wallet.locked_money -= tr.value;
                wallet.idle_money += tr.value;
                wallet.unused_money += (tr.pay_api == (byte)Lip2pEnums.PayApiTypeEnum.ManualAppend ? 0 : tr.value); // 手工充值 可能为活动返利，不计手续费
                wallet.total_charge += tr.value;
                wallet.last_update_time = tr.transact_time.Value; // 时间应该一致

                // 创建交易历史
                var his = CloneFromWallet(wallet, Lip2pEnums.WalletHistoryTypeEnum.ChargeConfirm);
                his.li_bank_transactions = tr;
                context.li_wallet_histories.InsertOnSubmit(his);
            }
            else if (tr.type == (int)Lip2pEnums.BankTransactionTypeEnum.Withdraw) // 提款确认
            {
                ...
                // 创建交易历史
                var his = CloneFromWallet(wallet, Lip2pEnums.WalletHistoryTypeEnum.WithdrawConfirm);
                his.li_bank_transactions = tr;
                context.li_wallet_histories.InsertOnSubmit(his);
            }
        ...
        /// <param name="saveChange"></param>
        /// <returns></returns>
        public static li_bank_transactions CancelBankTransaction(this Lip2pDataContext context, int bankTransactionId, int approver, bool saveChange = true)
        {
            // 更新原事务（完成事务）
            var tr = context.li_bank_transactions.Single(t => t.id == bankTransactionId);
            if (tr.status != (int) Lip2pEnums.BankTransactionStatusEnum.Acting)
                throw new InvalidOperationException("该银行卡" + Utils.GetLip2pEnumDes((Lip2pEnums.BankTransactionTypeEnum) tr.type) + "事务已经被确认或取消了");
            tr.status = (byte) Lip2pEnums.BankTransactionStatusEnum.Cancel;
            tr.transact_time = DateTime.Now;
            tr.approver = approver;

            if (tr.type == (int) Lip2pEnums.BankTransactionTypeEnum.Charge) // 充值取消
            {
                var wallet = tr.dt_users.li_wallets;
                // 修改钱包金额
                //wallet.locked_money -= tr.value;
                ...
                // 创建交易历史
                var his = CloneFromWallet(wallet, Lip2pEnums.WalletHistoryTypeEnum.ChargeCancel);
                his.li_bank_transactions = tr;
                context.li_wallet_histories.InsertOnSubmit(his);
            }
            else if (tr.type == (int) Lip2pEnums.BankTransactionTypeEnum.Withdraw) // 提款取消
            {
                var wallet = tr.li_bank_accounts.dt_users.li_wallets;
                // 修改钱包金额
                wallet.locked_money -= tr.value;
                wallet.idle_money += tr.value;
                if (tr.handling_fee_type == (int) Lip2pEnums.BankTransactionHandlingFeeTypeEnum.WithdrawUnusedMoneyHandlingFee)
                {
                ...
                // 创建交易历史
                var his = CloneFromWallet(wallet, Lip2pEnums.WalletHistoryTypeEnum.WithdrawCancel);
                his.li_bank_transactions = tr;
                context.li_wallet_histories.InsertOnSubmit(his);
            }
            if (saveChange) context.SubmitChanges();
        ...
        /// <returns></returns>
        public static li_wallet_histories CloneFromWallet(li_wallets wallet, Lip2pEnums.WalletHistoryTypeEnum actionType)
        {
        ...
        public static void Invest(this Lip2pDataContext context, int userId, int projectId, decimal investingMoney)
        {
            var pr = context.li_projects.Single(p => p.id == projectId);
            if (pr.tag == (int)Lip2pEnums.ProjectTagEnum.Trial)
            {
                InvestTrialProject(context, userId, projectId, investingMoney);
                return;
            }
            if (pr.tag == (int)Lip2pEnums.ProjectTagEnum.DailyProject)
            {
            ...
            var tr = new li_project_transactions
            {
                investor = userId,
                project = projectId,
                type = (byte) Lip2pEnums.ProjectTransactionTypeEnum.Invest,
                value = investingMoney,
                status = (byte) Lip2pEnums.ProjectTransactionStatusEnum.Success,
                create_time = wallet.last_update_time // 时间应该一致
            };
            context.li_project_transactions.InsertOnSubmit(tr);

            // 修改钱包历史
            var his = CloneFromWallet(wallet, Lip2pEnums.WalletHistoryTypeEnum.Invest);
        ...
        /// <param name="projectId"></param>
        /// <param name="investingMoney"></param>
        private static void InvestTrialProject(Lip2pDataContext context, int userId, int projectId, decimal investingMoney)
        {
            ...
            if (atr.status == (int) Lip2pEnums.ActivityTransactionStatusEnum.Acting)
            {
        ...
        private static void InvestDailyProject(Lip2pDataContext context, int userId, int projectId, decimal investingMoney) // TODO 未实现完
        {
        ...
        public static li_projects FinishInvestment(this Lip2pDataContext context, int projectId)
        {
            ...
            var repaymentType = (Lip2pEnums.ProjectRepaymentTypeEnum) project.repayment_type; // 还款类型
            ...
            if (repaymentType == Lip2pEnums.ProjectRepaymentTypeEnum.DengEr) // 等额本息
            {
                ...
                repaymentTasks = Enumerable.Range(1, termCount).Select(termNumber => new li_repayment_tasks
                {
                    ...
                    status = (byte) Lip2pEnums.RepaymentStatusEnum.Unpaid,
                    term = (short) termNumber,
                    should_repay_time = CalcRepayTime(project.invest_complete_time.Value, termSpan, termNumber, termCount)
                }).ToList();
            }
            else if (repaymentType == Lip2pEnums.ProjectRepaymentTypeEnum.XianXi) // 先息后本
            {
                repaymentTasks = Enumerable.Range(1, termCount).Select(termNumber => new li_repayment_tasks
                {
                    project = projectId,
        ...
        private static void CalcProfitingMoneyAfterRepaymentTasksCreated(this Lip2pDataContext context, li_projects project, List<li_repayment_tasks> tasks)
        {
            ...
            {
                var his = CloneFromWallet(w, Lip2pEnums.WalletHistoryTypeEnum.InvestSuccess);
        ...
        /// <returns></returns>
        public static DateTime CalcRepayTime(DateTime baseTime, Lip2pEnums.ProjectRepaymentTermSpanEnum termSpan, int termNumber, int termUnitCount)
        {
            switch (termSpan)
            {
                case Lip2pEnums.ProjectRepaymentTermSpanEnum.Year:
                    return baseTime.AddYears(termNumber);
                case Lip2pEnums.ProjectRepaymentTermSpanEnum.Month:
                    return baseTime.AddMonthsPreferLastDay(termNumber); // 月尾满标的话总是在月尾付息
                case Lip2pEnums.ProjectRepaymentTermSpanEnum.Day:
        ...
        /// <returns></returns>
        private static int CalcRealTermCount(Lip2pEnums.ProjectRepaymentTermSpanEnum termSpan, int termSpanCount)
        {
            switch (termSpan)
            {
                case Lip2pEnums.ProjectRepaymentTermSpanEnum.Year:
                ...
                case Lip2pEnums.ProjectRepaymentTermSpanEnum.Month:
                    return termSpanCount;
                case Lip2pEnums.ProjectRepaymentTermSpanEnum.Day: // 日的话只在最后一日还
                    return 1;
                default:
                    throw new InvalidEnumArgumentException("异常的项目还款跨度值");
            }
        ...
        /// <param name="termSpanCount"></param>
        /// <returns></returns>
        public static decimal CalcFinalProfitRate(DateTime baseTime, decimal profitRateYear, Lip2pEnums.ProjectRepaymentTermSpanEnum termSpanEnum, int termSpanCount)
        {
        ...
        public static readonly Dictionary<Lip2pEnums.ProjectTransactionTypeEnum, Lip2pEnums.WalletHistoryTypeEnum> PTrTypeToHisActTypeMap
            = new Dictionary<Lip2pEnums.ProjectTransactionTypeEnum, Lip2pEnums.WalletHistoryTypeEnum>
        {
            {Lip2pEnums.ProjectTransactionTypeEnum.RepaidPrincipalAndInterest, Lip2pEnums.WalletHistoryTypeEnum.RepaidPrincipalAndInterest},
            {Lip2pEnums.ProjectTransactionTypeEnum.RepaidPrincipal, Lip2pEnums.WalletHistoryTypeEnum.RepaidPrincipal},
            {Lip2pEnums.ProjectTransactionTypeEnum.RepaidInterest, Lip2pEnums.WalletHistoryTypeEnum.RepaidInterest}
        };
        ...
        public static li_repayment_tasks ExecuteRepaymentTask(this Lip2pDataContext context, int repaymentId, bool autoPaid)
        {
            var repaymentTask = context.li_repayment_tasks.Single(r => r.id == repaymentId);
            if (repaymentTask.status != (int) Lip2pEnums.RepaymentStatusEnum.Unpaid)
                throw new InvalidOperationException("这个还款计划已经执行过了");

            // 执行还款
            repaymentTask.status = autoPaid ? (byte)Lip2pEnums.RepaymentStatusEnum.AutoPaid: (byte)Lip2pEnums.RepaymentStatusEnum.ManualPaid;
            ...
            foreach (var ptr in ptrs)
            {
                ...
                // 添加钱包历史
                var his = CloneFromWallet(wallet, PTrTypeToHisActTypeMap[(Lip2pEnums.ProjectTransactionTypeEnum) ptr.type]);
            ...
            var newContext = new Lip2pDataContext(); // 旧的 context 有缓存，查询的结果不正确
        ...
        /// <param name="projectTransactionId"></param>
        /// <returns></returns>
        public static li_project_transactions Refund(this Lip2pDataContext context, int projectTransactionId)
        {
            // 判断项目状态是否未满标
            ...
            // 更改交易状态
            tr.status = (byte)Lip2pEnums.ProjectTransactionStatusEnum.Rollback;
            ...
            // 添加钱包历史
            var his = CloneFromWallet(wallet, Lip2pEnums.WalletHistoryTypeEnum.InvestorRefund);
            his.li_project_transactions = tr;
            context.li_wallet_histories.InsertOnSubmit(his);
        ...
        /// <returns></returns>
        public static T GetInvestmentProgress<T>(this Lip2pDataContext context, int projectId, Func<decimal, decimal, T> callback)
        {
        ...
        /// <param name="projectId"></param>
        /// <returns></returns>
        public static int GetInvestedUserCount(this Lip2pDataContext context, int projectId)
        {
            var pro = context.li_projects.Single(a => a.id == projectId);
            return pro.li_project_transactions.Where(t => t.type == (int) Lip2pEnums.ProjectTransactionTypeEnum.Invest &&
        ...
        /// <param name="context"></param>
        /// <returns></returns>
        public static decimal QueryTotalProfit(this Lip2pDataContext context)
        {
        ...
        /// <param name="context"></param>
        /// <returns></returns>
        public static decimal QueryTotalInvested(this Lip2pDataContext context)
        {
        ...
        /// <param name="context"></param>
        /// <returns></returns>
        public static decimal QueryTotalInvesting(this Lip2pDataContext context)
        {
        ...
        /// <returns></returns>
        public static decimal QueryTradingVolume(this Lip2pDataContext context, int inDaysEarlier)
        {
            var now = DateTime.Now;
            return context.li_project_transactions.Where(
                r =>
                    r.status == (int) Lip2pEnums.ProjectTransactionStatusEnum.Success && r.type == (int)Lip2pEnums.ProjectTransactionTypeEnum.Invest &&
        ...
        public static T QueryTransactionIncome<T>(li_wallet_histories his, Func<decimal?, decimal?, T> callback)
        {
            ...
            {
                var chargedValue = his.li_bank_transactions.type == (int) Lip2pEnums.BankTransactionTypeEnum.Withdraw
                    ? (decimal?) null
            ...
            if (his.li_project_transactions != null)
            {
                ...
                {
                    case (int)Lip2pEnums.ProjectTransactionTypeEnum.Invest:
                        receivedPrincipal = profited = null;
                        break;
                    case (int) Lip2pEnums.ProjectTransactionTypeEnum.RepaidInterest:
            ...
            if (his.li_activity_transactions != null && his.li_activity_transactions.type == (int) Lip2pEnums.ActivityTransactionTypeEnum.Lost)
            {
        ...
        public static decimal? QueryTransactionOutcome(li_wallet_histories his)
        {
            if (his.li_bank_transactions != null)
            {
                return his.li_bank_transactions.type == (int)Lip2pEnums.BankTransactionTypeEnum.Charge
                    ? (decimal?)null
                    : his.li_bank_transactions.value; // 提现
            }
            if (his.li_project_transactions != null)
            {
                if (his.action_type == (int) Lip2pEnums.WalletHistoryTypeEnum.InvestSuccess) // 项目满标不显示支出
                    return null;
                return his.li_project_transactions.type == (int)Lip2pEnums.ProjectTransactionTypeEnum.Invest
                    ? his.li_project_transactions.value // 投资
                    : (decimal?)null;
            }
            // 活动扣除
            if (his.li_activity_transactions != null && his.li_activity_transactions.type == (int)Lip2pEnums.ActivityTransactionTypeEnum.Gain) return null;
        ...
        {
            return Utils.GetLip2pEnumDes((Lip2pEnums.ProjectRepaymentTermSpanEnum) proj.repayment_term_span);
        }
    }
}