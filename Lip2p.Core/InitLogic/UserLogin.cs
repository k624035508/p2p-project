...
namespace Lip2p.Core.InitLogic
{
    class UserLogin
    {
        ...
        private static void NewUser(int userId, bool remember, string code, Action doLogin)
        {
            ...
            if (remember)
            {
                Utils.WriteCookie(DTKeys.COOKIE_USER_NAME_REMEMBER, "Lip2p", user.user_name, 43200, true);
                Utils.WriteCookie(DTKeys.COOKIE_USER_PWD_REMEMBER, "Lip2p", user.password, 43200, true);
            }
            else
            {
                //防止Session提前过期
                Utils.WriteCookie(DTKeys.COOKIE_USER_NAME_REMEMBER, "Lip2p", user.user_name, true);
                Utils.WriteCookie(DTKeys.COOKIE_USER_PWD_REMEMBER, "Lip2p", user.password, true);
            }
