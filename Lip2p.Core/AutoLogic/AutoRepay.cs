    ...
    class AutoRepay
    {
        ...
        private static void DoRepay(bool onTime)
        {
            ...
            var shouldRepayTask = context.li_repayment_tasks.Where(
                t =>
                    t.status == (int) Lip2pEnums.RepaymentStatusEnum.Unpaid &&
                    t.should_repay_time.Date <= DateTime.Today).ToList();
            shouldRepayTask.ForEach(ta => context.ExecuteRepaymentTask(ta.id, true));
        ...
        private static void SendRepayNotice(List<li_repayment_tasks> task, Lip2pDataContext context)
        {
            //找到兑付公告模板
