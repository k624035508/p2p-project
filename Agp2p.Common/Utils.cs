...
namespace Agp2p.Common
{
    public static class Utils
    {
        ...
        public static string GetOrderNumberLonger()
        {
            return DateTime.Now.ToString("yyyyMMddHHmmssfff") + Number(2, true);
        }
        ...
        public static string DropHTMLStr(string Htmlstring)
        {
            if (string.IsNullOrEmpty(Htmlstring)) return "";
            //删除脚本  
            Htmlstring = Regex.Replace(Htmlstring, @"<script[^>]*?>.*?</script>", "", RegexOptions.IgnoreCase);
        ...
        public static string GetXmlMapPath(string xmlName, bool loadOutsideProject = false)
        {
            return GetMapPath(ConfigurationManager.AppSettings[xmlName], loadOutsideProject);
        }
        #endregion
        ...
        public static string GetMapPath(string strPath, bool loadOutsideProject = false)
        {
            if (strPath.ToLower().StartsWith("http://"))
            {
                return strPath;
            }
            if (HttpContext.Current != null && !loadOutsideProject)
            {
        ...
        public static void WriteCookie(string strName, string strValue, bool httpOnly = false)
        {
        ...
        /// <param name="strName">名称</param>
        /// <param name="strValue">值</param>
        public static void WriteCookie(string strName, string key, string strValue, bool httpOnly = false)
        {
            HttpCookie cookie = HttpContext.Current.Request.Cookies[strName];
            if (cookie == null)
            {
        ...
        /// <param name="strName">名称</param>
        /// <param name="strValue">值</param>
        public static void WriteCookie(string strName, string key, string strValue, int expires, bool httpOnly = false)
        {
            HttpCookie cookie = HttpContext.Current.Request.Cookies[strName];
            if (cookie == null)
            {
        ...
        public static void WriteCookie(string strName, string strValue, int expires, bool httpOnly = false)
        {
            HttpCookie cookie = HttpContext.Current.Request.Cookies[strName];
            if (cookie == null)
            {
        ...
        public static string CombUrlTxt(string _url, string _keys, params string[] _values)
        {
            ...
            try
            {
                string[] keyArr = _keys.Split('&');
                for (int i = 0; i < keyArr.Length; i++)
                {
                    if (keyArr[i].StartsWith("page=") && _values[i] == "0") continue;
        ...
        /// <param name="param"></param>
        /// <returns></returns>
        public static string HttpPostByte(string url, string param)
        {
            HttpWebRequest request = (HttpWebRequest)HttpWebRequest.Create(url);
            request.Method = "POST";
            request.ContentType = "application/x-www-form-urlencoded";
            ...
            try
            {
                ...
                requestStream = request.GetRequestStream();
                requestStream.Write(arrB, 0, arrB.Length);
                requestStream.Close();
        ...
        /// <returns></returns>
        public static string GetAgp2pEnumDes(Enum en)
        {
        ...
        public static string GetAgp2pEnumDes<T>(string intStr)
        {
            return GetAgp2pEnumDes(Enum.Parse(typeof (T), intStr) as Enum);
        }

        public static IEnumerable<int> Infinite(int start = 0)
        {
            while (true)
                yield return start++;
        }

        public static NameValueCollection ParceQueryString()
        {
            ...
            var queryString = url.Substring(startIndex).Split('#')[0];
        ...
        public static string GetUserNameHidden(string user_name)
        {
            if (user_name.Length >= 4)
                user_name = user_name.Substring(0, 2) + "**" + user_name.Substring(user_name.Length - 2, 2);
            return user_name;
        }
        ...
        public static Dictionary<TKey, TVal> Merge<TKey, TVal>(this Dictionary<TKey, TVal> src, Dictionary<TKey, TVal> dict)
        {
            return src.Concat(dict).GroupBy(p => p.Key).ToDictionary(g => g.Key, g => g.Last().Value);
        }
        ...
        public static List<decimal> GetPerfectRounding(List<decimal> original, decimal forceSum, int decimals)
        {
            ...
            if (delta < 0)
            {
                applyDeltaSequence = original
                    .Zip(Infinite(), (x, index) => new { x, index })
