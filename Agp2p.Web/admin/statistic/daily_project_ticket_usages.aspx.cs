    ...
    public partial class daily_project_ticket_usages : UI.ManagePage
    {
            ...
            public UsageItem(dt_users u)
            {
                ...
                _expiredTicketCount = new Lazy<int>(() =>
                {
                    return u.li_activity_transactions.Count(atr =>
                                ...
                                atr.status == (int) Agp2pEnums.ActivityTransactionStatusEnum.Cancel);
                });
                _unuseTicketCount = new Lazy<int>(() =>
                {
                    return u.li_activity_transactions.Count(atr =>
                                atr.activity_type == (int) Agp2pEnums.ActivityTransactionActivityTypeEnum.DailyProject &&
                                atr.status == (int) Agp2pEnums.ActivityTransactionStatusEnum.Acting);
                });
                _usedTicketCount = new Lazy<int>(() =>
                {
                    return u.li_activity_transactions.Count(atr =>
                                atr.activity_type == (int) Agp2pEnums.ActivityTransactionActivityTypeEnum.DailyProject &&
                                atr.status == (int) Agp2pEnums.ActivityTransactionStatusEnum.Confirm && atr.transact_time == null);
                });
        ...
        private List<UsageItem> QueryUsageItem()
        {
            ...
            var users = context.dt_users.Where(w => !canAccessGroups.Any() || canAccessGroups.Contains(w.group_id))
                ...
                .ToList();
            return users.Where(u => u.li_activity_transactions.Any(atr => atr.activity_type == (int) Agp2pEnums.ActivityTransactionActivityTypeEnum.DailyProject))
