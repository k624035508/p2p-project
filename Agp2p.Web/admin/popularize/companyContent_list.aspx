<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="companyContent_list.aspx.cs" Inherits="Agp2p.Web.admin.popularize.companyContent_list" %>
...
<div class="toolbar-wrap">
  <div id="floatHead" class="toolbar">
    <div class="l-list">
      <ul class="icon-list">
        <li><a class="add" href="companyContent_edit.aspx?action=<%=DTEnums.ActionEnum.Add %>&channel_id=<%=this.channel_id %>&category_id=<%=this.category_id %>"><i></i><span>新增</span></a></li>
...
<ItemTemplate>
  <tr>
    ...
    <td>
      <div class="btn-tools">
        <asp:LinkButton ID="lbtnIsMsg" CommandName="lbtnIsMsg" runat="server" CssClass='<%# Eval("is_msg").ToString() != "" && Convert.ToInt32(Eval("is_msg")) == 1 ? "msg selected" : "msg"%>' ToolTip='<%# Eval("is_msg").ToString() != "" && Convert.ToInt32(Eval("is_msg")) == 1 ? "取消评论" : "设置评论"%>' />
        <asp:LinkButton ID="lbtnIsTop" CommandName="lbtnIsTop" runat="server" CssClass='<%# Eval("is_msg").ToString() != "" && Convert.ToInt32(Eval("is_top")) == 1 ? "top selected" : "top"%>' ToolTip='<%# Eval("is_msg").ToString() != "" && Convert.ToInt32(Eval("is_top")) == 1 ? "取消置顶" : "设置置顶"%>' />
        <asp:LinkButton ID="lbtnIsRed" CommandName="lbtnIsRed" runat="server" CssClass='<%# Eval("is_msg").ToString() != "" && Convert.ToInt32(Eval("is_red")) == 1 ? "red selected" : "red"%>' ToolTip='<%# Eval("is_msg").ToString() != "" && Convert.ToInt32(Eval("is_red")) == 1 ? "取消推荐" : "设置推荐"%>' />
        <asp:LinkButton ID="lbtnIsHot" CommandName="lbtnIsHot" runat="server" CssClass='<%# Eval("is_msg").ToString() != "" && Convert.ToInt32(Eval("is_hot")) == 1 ? "hot selected" : "hot"%>' ToolTip='<%# Eval("is_msg").ToString() != "" && Convert.ToInt32(Eval("is_hot")) == 1 ? "取消热门" : "设置热门"%>' />
...
<ItemTemplate>
    <li>
      <div class="details<%#Eval("img_url").ToString() != "" ? "" : " nopic"%>">
        ...
        <h1><span><a href="companyContent_edit.aspx?action=<%#DTEnums.ActionEnum.Edit %>&channel_id=<%#this.channel_id %>&id=<%#Eval("id")%>"><%#Eval("title")%></a></span></h1>
        <div class="remark">
        ...
        <div class="foot">
          ...
          <a href="companyContent_edit.aspx?action=<%#DTEnums.ActionEnum.Edit %>&channel_id=<%#this.channel_id %>&id=<%#Eval("id")%>" title="编辑" class="edit">编辑</a>
        </div>
      </div>
    </li>
</ItemTemplate>
