...
namespace Agp2p.Test
{
    ...
    public class UnitTest_Transaction
    {
        ...
        {
            Agp2pDataContext context = new Agp2pDataContext(str);
            var projects = context.li_projects.Where(p => (int)Agp2pEnums.ProjectStatusEnum.Financing <= p.status && p.investment_amount == 0).ToList();
            ...
            var projects =
                context.li_projects.Where(
                    p =>
                        ...
                        p.tag != (int) Agp2pEnums.ProjectTagEnum.Trial && p.tag != (int)Agp2pEnums.ProjectTagEnum.DailyProject).ToList();
            int doneCount = 0;
            projects.ForEach(p =>
            {
                var investmentAmount = p.li_project_transactions.Where(
            ...
            var errorProjects =
                context.li_projects.Where(
                    p =>
                        ...
                        p.li_repayment_tasks.All(r => r.status != (int) Agp2pEnums.RepaymentStatusEnum.Unpaid)).ToList();
        ...
        public void FixTotalReChargeAndWithDraw() // 计算累计充值和累计提现
        {
            ...
            {
                List<li_bank_transactions> transactions = context.li_bank_transactions.Where(b => b.charger == w.user_id && b.type == (int)Agp2pEnums.BankTransactionTypeEnum.Charge &&
                b.status == (int)Agp2pEnums.BankTransactionStatusEnum.Confirm).ToList();
                List<li_activity_transactions> activity_transactions = context.li_activity_transactions.Where(a => a.user_id == w.user_id &&
                    a.activity_type == (int)Agp2pEnums.ActivityTransactionActivityTypeEnum.ManualOperation
                    && a.type == (int)Agp2pEnums.ActivityTransactionTypeEnum.Gain).ToList();
            ...
            walletList.ForEach(w =>
            {
                ...
                && b.type == (int)Agp2pEnums.BankTransactionTypeEnum.Withdraw
                && b.status == (int)Agp2pEnums.BankTransactionStatusEnum.Confirm).ToList();
        ...
        public void UpdateOldUserHongbao() // 为以前未投资过的用户发放红包
        {
            ...
            var userList =
                context.li_wallets.Where(
                    ...
                    .Select(w => w.dt_users)
                    .AsEnumerable()
                    .Where(u => !u.li_activity_transactions.Any(a => a.activity_type == (int) Agp2pEnums.ActivityTransactionActivityTypeEnum.HongBaoActivation))
                    .ToList();
            ...
            userList.ForEach(u => 
            {
                ...
                if (new DateTime(2015, 5, 3) <= DateTime.Now && DateTime.Now <= deadline)
                {
                    var trs = HongbaoActivateMoneyMap.Select(m => new li_activity_transactions
                    {
                        ...
                        activity_type = (byte)Agp2pEnums.ActivityTransactionActivityTypeEnum.HongBaoActivation,
                    });
        ...
        private static List<KeyValuePair<dt_users, decimal>> QueryAbnormalProfitingMoneyList(Agp2pDataContext context)
        {
            // 筛选出等待还款的用户
            ...
            var sumOfProfitingMap = users.ToDictionary(u => u, u =>
            {
                var projectInvestmentMap = u.li_project_transactions.Where(p =>
                    p.status == (int) Agp2pEnums.ProjectTransactionStatusEnum.Success &&
                    p.type == (int) Agp2pEnums.ProjectTransactionTypeEnum.Invest &&
                    p.li_projects.status < (int) Agp2pEnums.ProjectStatusEnum.RepayCompleteIntime)
                ...
                var sumOfProfiting = projectInvestmentMap.Keys.SelectMany(
                    p =>
                        p.li_repayment_tasks.Where(
                            t => t.status == (int) Agp2pEnums.RepaymentStatusEnum.Unpaid && 0 < t.repay_interest)).Sum(
                                t =>
                                {
        ...
        public void FixInvestSuccessHistory() // 幂等：修正投资完成历史的创建时间和关联投资记录 (创建时间应等于应等于项目满标时间，投资记录是用户最后一次对某项目的投资记录)
        {
            ...
            var users = context.dt_users.Where(u =>
                u.li_wallet_histories.Any(h =>
                        h.action_type == (int) Agp2pEnums.WalletHistoryTypeEnum.InvestSuccess &&
                ...
                prematchHis.ForEach(h =>
                {
                    ...
                    var projectProfitMap = projs.ToDictionary(p => p, proj =>
                    {
                        ...
                        var investments = proj.li_project_transactions.Where(
                            tr =>
                                tr.investor == u.id && tr.type == (int) Agp2pEnums.ProjectTransactionTypeEnum.Invest &&
                                tr.status == (int) Agp2pEnums.ProjectTransactionStatusEnum.Success).ToList();
        ...
        public void QueryProfitingMoneyMistakeProject() // 查询投资和满标都加了待收利息的项目
        {
            ...
            context.li_wallet_histories.Where(
                ...
                .Where(g => g.Any(h => h.action_type == (int)Agp2pEnums.WalletHistoryTypeEnum.InvestSuccess))
                .ToDictionary(g => g.Key, g => g.GroupBy(h => h.dt_users).First())
                .Where(m =>
                {
                    var firstInvestment = m.Value.First(h => h.action_type == (int)Agp2pEnums.WalletHistoryTypeEnum.Invest);
        ...
        public void FixHistoryAboutProfitingMoney() // 幂等
        {
            ...
            investers.ForEach(i =>
            {
                ...
                for (int index = 0; index < userHis.Count; index++)
                {
                    ...
                    if (curr.action_type == (int) Agp2pEnums.WalletHistoryTypeEnum.Invest && curr.li_project_transactions.project == proj.id)
                    {
        ...
        {
            var context = new Agp2pDataContext(str);
            var tasks = context.li_repayment_tasks.Where(t => t.status != (int) Agp2pEnums.RepaymentStatusEnum.Unpaid).ToList();
            ...
            tasks.ForEach(t =>
            {
                ...
                var repaidInvestor = context.li_project_transactions.Where(
                    tr => tr.type != (int) Agp2pEnums.ProjectTransactionTypeEnum.Invest &&
                          tr.status == (int) Agp2pEnums.ProjectTransactionStatusEnum.Success &&
                ...
                if (investers.Count() != repaidInvestor.Count)
                {
                    ...
                    unpaidUser.ForEach(userId =>
                    {
                        ...
                        // 添加钱包历史
                        var his = TransactionFacade.CloneFromWallet(wallet, TransactionFacade.PTrTypeToHisActTypeMap[(Agp2pEnums.ProjectTransactionTypeEnum)ptr.type]);
                ...
                for (int i = 1; i < his.Count; i++)
                {
                    ...
                    if (0 < increasement)
                    {
                        if (curr.create_time < new DateTime(2015, 5, 3, 18, 0, 0))
                        {
                            ...
                            var assest = curr.action_type == (int) Agp2pEnums.WalletHistoryTypeEnum.Invest ||
                            ...
                            Debug.Assert(curr.action_type == (int)Agp2pEnums.WalletHistoryTypeEnum.InvestSuccess); // TODO 修正不正常的项目满标（由于不在正常时间完成项目导致的）
                        ...
                        var investments = user.li_project_transactions.Where(tr =>
                                    ...
                                    (int)Agp2pEnums.ProjectTransactionTypeEnum.Invest == tr.type &&
                                    tr.status == (int)Agp2pEnums.ProjectTransactionStatusEnum.Success).ToList();
                        // 取得用户的实际收益，修正到增长
                        ...
                        {
                            if (curr.action_type == (int) Agp2pEnums.WalletHistoryTypeEnum.InvestSuccess) continue; // 暂时忽略新平台的多次投资
                ...
                var atr =
                    firstInvestProfits.ToList().Where(d =>
                    {
                        var granted = d.Key.dt_users1.li_activity_transactions.Any(
                            a =>
                                a.activity_type ==
                                (int) Agp2pEnums.ActivityTransactionActivityTypeEnum.RefereeFirstTimeProfitBonus &&
                    ...
                    }).Select(d => new li_activity_transactions // 邀请人创建 activity_transaction
                    {
                        ...
                        status = (byte) Agp2pEnums.ActivityTransactionStatusEnum.Acting,
                        activity_type =
                            (byte) Agp2pEnums.ActivityTransactionActivityTypeEnum.RefereeFirstTimeProfitBonus,
                    });
        ...
        {
            var db = new Agp2pDataContext(str);
